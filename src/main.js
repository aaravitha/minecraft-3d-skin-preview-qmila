...
// Show real errors in the status box instead of freezing on "Loading viewer…"
...
const DEFAULT_SKIN_URL = "/default-skin.png";
...
// classic/slim is applied by re-loading skin with model option
...
function clearSkins() {
  ...
  // clear persisted last skin + any saved settings if you want (keep settings by default)
  localStorage.removeItem(LS.lastSkin);

  // clear bulk thumbnails & revoke object URLs
  bulk.forEach((x) => { try { URL.revokeObjectURL(x.url); } catch {} });
  bulk = [];
  bulkIndex = -1;
  bulkGrid.innerHTML = "";
  ...
  // reset state and load default skin again
  paused = false;
  playPauseBtn.textContent = "Pause animation";
  setStatus("Cleared skins. Loading default…");
  loadSkinUrl(DEFAULT_SKIN_URL);
}
...
// NEW: clear skins button
clearSkinsBtn.addEventListener("click", () => {
  if (!confirm("Clear uploaded skins and restore default skin?")) return;
  clearSkins();
});
...
applyBackground();
controls.autoRotate = autoRotateEl.checked;
setStatus("Viewer ready. Loading default skin…");

// Load last skin if available, otherwise load default from /public/default-skin.png
restoreLastSkinIfAny().then((restored) => {
  if (!restored) loadSkinUrl(DEFAULT_SKIN_URL);
});
